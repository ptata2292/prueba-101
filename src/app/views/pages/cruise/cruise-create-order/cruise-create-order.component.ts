...
@Component({
  selector: 'app-cruise-create-order',
  templateUrl: './cruise-create-order.component.html',
  styleUrls: ['./cruise-create-order.component.css']
})
export class CruiseCreateOrderComponent extends Cruise implements OnInit {
  ...
  bookFlight(){
    ...
    if(this.passengerAFG.valid && this.contactFG.valid && (this.paymentType == this.paymentTypes[0] || this.creditCardFG.valid)) {
      ...
      this.prepareQuery();
      this.isLoading = true;
      this.cruiseOrderService.createOrder(this.query).subscribe((data: any) => {
        ...
        if(data.errors!= null && data.errors.error!= null) {
          ...
          if(data != null && data.response!= null && data.response.order != null && data.response.order.orderID != null) {
            ...
            //this.router.navigateByUrl('/Cruises/Orders');
            //this.router.navigateByUrl('/');
            this.router.navigateByUrl('/Cruises/Status/'+ data.response.order.orderID);
          }         
        }
      });
    } else {
  ...
  prepareQuery() {
    ...
    /*this.cruiseOrderService.gerOrderById('AA0018281EC52').subscribe((data: any) => {
