...
import { CruiseItem } from './cruise-item/cruise-item';
...
export class CruiseListComponent extends Cruise implements OnInit, AfterViewInit, OnDestroy {
  ...
  constructor(private cruiseRetrieveService: CruiseRetrieveService, private router: Router, 
  ...
  searchCruise(query) {
    ...
    this.cruiseRetrieveService.searchCruises(query).
      subscribe((data: any) => {
  ...
  bookNow() {
    ...
    this.router.navigateByUrl('/Cruises/Booking');
  }
  ...
  createFinalizedOffers(offer) {
    ...
    for (let flightsPlusSegment of offer.flightsPlusSegments) {
      let flightSegments = flightsPlusSegment.flightSegments.map(segment => {
        return {
          cruiseID: segment.flightSegment.marketingCarrier.cruiseID,
          ...
          cruiseName: segment.flightSegment.marketingCarrier.name
        }
      });
      let flightFareOld = {
        totalPrice: (Number(offer.totalPrice.detailCurrencyPrice.total.text) * 0.01),
