...
import { SetSearchCriteriaData } from '../../../../store/cruise/cruise.actions';
import { CruiseStateModule } from '../../../../store/cruise/cruise.state';
import { MyErrorStateMatcher, trip } from '../../../../shared/models/cruise.model';
import { LocalStorage } from '../../../../shared/storage/local-storage';

@Component({
  selector: 'app-cruise-search',
  templateUrl: './cruise-search.component.html',
  styleUrls: ['./cruise-search.component.css']
})

export class CruiseSearchComponent extends Cruise implements OnInit, AfterViewInit, OnDestroy {
  public response = {};
  public cruiseData = {};
  public cruiseLoading = false;
  ...
  @ViewChild('cruiseListComponent', {static : false}) cruiseListComponentRef: CruiseListComponent;
  private docClickSubscription: any;
  public isLoading = false;
  public isSubmitted = false;
  @Select(CruiseStateModule.getSearchCriteria) searchCriteria$: Observable<any>;
  searchSubject: Subject<any> = new Subject<any>();

  constructor(loginService: LoginService, private cruiseRetrieveService: CruiseRetrieveService,
  ...
  ngOnInit() {
    ...
    this.filteredToOptions = this.searchFormGroup.get('to').valueChanges
      .pipe(
        debounceTime(500),
        switchMap((cruiseCode) => {
          if(this.filter(cruiseCode)){
            return of([]);
          } else {
            this.loadingLeavingTo = true;
            return this.serachAirportSearchService(cruiseCode);
          }
      }));
    ...
    /* this.filteredToOptions = this.searchFormGroup.get('to').valueChanges
      .pipe(
        switchMap((cruiseCode) => {
          if(cruiseCode == '' || cruiseCode == null ) {
            return of(this.airPortList);
          } else {
            this.loadingLeavingTo = true;
            return this.serachOfflineAirportSearch(cruiseCode);
          }
      }));
    this.filteredFromOptions = this.searchFormGroup.get('from').valueChanges
      .pipe(
  ...
  public ngAfterViewInit() {
    // this.cruiseListComponentRef.searchCruise(this.getSearchQuery());
    // console.log('ngAfterViewInit');
    this.emitEventToCruiseList();
  }
  ...
  serachAirportSearchService(cruiseCode) {
    const code = cruiseCode.data != null ? cruiseCode.data : cruiseCode;
    return this.cruiseRetrieveService.searchAirport(code)
            .pipe(map((data: any) => {
  ...
  serachOfflineAirportSearch(cruiseCode) {
    let suggestions = [];
    if(this.airPortList.length == 0){
      ...
      this.loadingLeavingTo = false;
      this.loadingLeavingFrom = false;
      const filterAirports = this.filterAirport(cruiseCode);
      return of(filterAirports);
    }    
  }

  filterAirport(cruiseCode) {
    let suggestions = [];
    let airPortCode = cruiseCode.name || cruiseCode.value;
    airPortCode = airPortCode == null ? cruiseCode : airPortCode;
  ...
  getAllAirportSearchService() {
    this.cruiseRetrieveService.getAllAirport().
      subscribe((data: any) => {
  ...
  serachCruisesService() {
      this.cruiseLoading = true;
      this.cruiseRetrieveService.searchCruises(this.getSearchQuery()).
          subscribe((data: any) => {
  ...
  filter(cruiseCode) {
    if(cruiseCode == '' || cruiseCode == null ){
      return true;
    } else if(cruiseCode.data != null){
      return false;
    } else if (cruiseCode.length >= 1){
      return false;
    } else {
      return true;
    }
