...
import { SpaItem } from './spa-item/spa-item';
...
export class SpaListComponent extends Spa implements OnInit, AfterViewInit, OnDestroy {
  ...
  constructor(private spaRetrieveService: SpaRetrieveService, private router: Router, 
  ...
  searchSpa(query) {
    ...
    this.spaRetrieveService.searchSpas(query).
      subscribe((data: any) => {
  ...
  bookNow() {
    ...
    this.router.navigateByUrl('/Spas/Booking');
  }
  ...
  createFinalizedOffers(offer) {
    ...
    for (let flightsPlusSegment of offer.flightsPlusSegments) {
      let flightSegments = flightsPlusSegment.flightSegments.map(segment => {
        return {
          spaID: segment.flightSegment.marketingCarrier.spaID,
          ...
          spaName: segment.flightSegment.marketingCarrier.name
        }
      });
      let flightFareOld = {
        totalPrice: (Number(offer.totalPrice.detailCurrencyPrice.total.text) * 0.01),
