...
@Component({
  selector: 'app-spa-create-order',
  templateUrl: './spa-create-order.component.html',
  styleUrls: ['./spa-create-order.component.css']
})
export class SpaCreateOrderComponent extends Spa implements OnInit {
  ...
  constructor(private router: Router, loginService: LoginService, private localStorage: LocalStorage,
    ...
    if(selectedFlightTxt != null && searchCriteriaTxt != null) {
      ...
      this.router.navigateByUrl('/Spa/Search');
    }
  }
  ...
  bookFlight(){
    ...
    if(this.passengerAFG.valid && this.contactFG.valid && (this.paymentType == this.paymentTypes[0] || this.creditCardFG.valid)) {
      ...
      this.prepareQuery();
      this.isLoading = true;
      this.spaOrderService.createOrder(this.query).subscribe((data: any) => {
        ...
        if(data.errors!= null && data.errors.error!= null) {
          ...
          if(data != null && data.response!= null && data.response.order != null && data.response.order.orderID != null) {
            ...
            //this.router.navigateByUrl('/Spas/Orders');
            //this.router.navigateByUrl('/');
            this.router.navigateByUrl('/Spas/Status/'+ data.response.order.orderID);
          }         
        }
      });
    } else {
  ...
  prepareQuery() {
    ...
    /*this.spaOrderService.gerOrderById('AA0018281EC52').subscribe((data: any) => {
