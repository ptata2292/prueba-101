...
@Component({
  selector: 'app-spa-orders',
  templateUrl: './spa-orders.component.html',
  styleUrls: ['./spa-orders.component.css']
})
export class SpaOrdersComponent extends Spa implements OnInit, AfterViewInit  {
  ...
  search() {
    ...
    if(this.searchOrderFG.valid) {
      ...
      this.isLoading = true;
      this.spaOrderService.gerOrderByName(value.FirstName, value.LastName).subscribe((data: any) => {
  ...
  searchById() {
    ...
    if(this.searchOrderIdFG.valid) {
      ...
      this.isLoading = true;
      this.spaOrderService.gerOrderById(value.OrderId).subscribe((data: any) => {
        // console.log('gerOrderByName');
        // console.log(data);
        this.isLoading = false;
        let order = {
